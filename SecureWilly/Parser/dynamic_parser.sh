...
for SERVICE in "${service_list[@]}"; do
	...
	if [[ "$static_part" > "0" ]]; then
		...
		python2.7 ${dynamic_script_path}/create_version_1.py ${SERVICE}
	fi
done
...
while [[ "$enforce" == "0" ]]; do
		...
		for SERVICE in "${service_list[@]}"; do
			...
			if $abort_net ; then
				...
				if [[ "$if_net" != "0" ]]; then
					...
					python2.7 ${dynamic_script_path}/1_abort_network_rule.py $SERVICE
				fi
			fi
			...
			if [[ "$num_vols" == "0" ]]; then
				python2.7 ${dynamic_script_path}/11_merge_profiles.py $SERVICE $i 'complain'
			else
	...
	for SERVICE in "${service_list[@]}"; do 
		...
		num_vols=$(cut -d'%' -f2 if_vol_${SERVICE})
		if [[ "$num_vols" == "0" ]]; then
			python2.7 ${dynamic_script_path}/11_merge_profiles.py $SERVICE $i 'enforce'
		else
			sed -i "83s/#/ /" ${dynamic_script_path}/11_merge_profiles.py
			sed -i "83s|if .*|if ${vol_str}|" ${dynamic_script_path}/11_merge_profiles.py
			sed -i "84s/#/ /" ${dynamic_script_path}/11_merge_profiles.py
