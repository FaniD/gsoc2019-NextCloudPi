...
#pwd and pre_pwd are given as they are because we need paths, not names, just add "" to make it a string in python2.7
pwd_path=$(pwd | sed "s,.*,\"&\",")
...
for service_i in "${array[@]}"; do
	...
	python2.7 find_vols.py ${array_noslash[${yml_count}]}_yml
	
	#This file is needed in dynamic_analysis
	mv if_vol if_vol_${array_noslash[${yml_count}]}

	#Run static parser
	python2.7 static_parser.py ${array_noslash[${yml_count}]}_dockerfile_path ${array_noslash[${yml_count}]}_yml
...
if [[ "$nethost" != "0" ]]; then
	python2.7 alert.py network "enters host's Network namespace."
	if [ -e yml_alert_net ]
	then
...
if [[ "$pidhost" != "0" ]]; then
        python2.7 alert.py pid "enters host's PID namespace."
	if [ -e yml_alert_pid ]
        then
...
if [[ "$utshost" != "0" ]]; then
        python2.7 alert.py uts "enters host's UTS namespace."
        awk '!seen[$0]++' alert_logs > alert_logs_
        cat alert_logs_ >> ${app_run_path}/parser_output/Alerts/Namespaces
        rm alert_logs
        echo "" >> ${app_run_path}/parser_output/Alerts/Namespaces
...
if [[ "$ipchost" != "0" ]]; then
        python2.7 alert.py ipc "enters host's IPC namespace."
        awk '!seen[$0]++' alert_logs > alert_logs_
        cat alert_logs_ >> ${app_run_path}/parser_output/Alerts/Namespaces
        rm alert_logs
        echo "" >> ${app_run_path}/parser_output/Alerts/Namespaces
...
if [[ "$usrhost" != "0" ]]; then
        python2.7 alert.py userns "enters host's User namespace."
	if [ -e yml_alert_usr ]
        then
...
if [[ "$priv" != "0" ]]; then
        python2.7 alert.py privileged "runs in privileged mode."
        if [ -e yml_alert_priv ]
        then
