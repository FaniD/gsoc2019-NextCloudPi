...
#  docker-machine ip worker${i}
  docker-machine ssh worker${i} "sudo mount --bind /var/lib/docker/volumes/NCP_ncdata/_data /var/lib/docker/volumes/NCP_ncdata/_data"
  docker-machine ssh worker${i} "sudo mount --make-shared /var/lib/docker/volumes/NCP_ncdata/_data"
###
  docker-machine ssh worker${i} "docker run --restart=always --name gfsc${i} -v /bricks:/bricks -v /etc/glusterfs:/etc/glusterfs:z -v /var/lib/glusterd:/var/lib/glusterd:z -v /var/log/glusterfs:/var/log/glusterfs:z -v /sys/fs/cgroup:/sys/fs/cgroup:ro --mount type=bind,source=/var/lib/docker/volumes/NCP_ncdata/_data,target=/var/lib/docker/volumes/NCP_ncdata/_data,bind-propagation=rshared -d --privileged=true --net=netgfsc -v /dev/:/dev gluster/gluster-centos"
...
docker exec -it gfsc0 mount.glusterfs gfsc0:/gv0 /var/lib/docker/volumes/NCP_ncdata/_data
